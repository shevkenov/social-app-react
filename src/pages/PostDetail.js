...
export default function PostDetail() {
  ...
  const {data: post, isLoading } = useQuery("PostDetails", () => getPostById(postId));
  const {data: replies, isSuccess } = useQuery("PostReplies", () => getReplies(postId));

  if(isLoading) return <Spinner />;

  if(!post) return <div className="message font-weight-bold">Post not found</div>
