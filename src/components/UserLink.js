...
export default function UserLink({ ...props }) {
  ...
  return (
    <OverlayTrigger placement="auto" delay="300" show={show} overlay={<UserPopover setShow={setShow} user={props.user}/>}>
      <Link {...props} />
    </OverlayTrigger>
  );
}
...
const UserPopover = React.forwardRef(({user, setShow, ...props }, ref) => {
  ...
  return (
    <Popover className="border-0" ref={ref} id="user-popover" {...props}>
      <Card onMouseOver={() => setShow(true)} onMouseLeave={() => setShow(undefined)} className="border p-3 bg-transparent m-0">
