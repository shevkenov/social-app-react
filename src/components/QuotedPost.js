...
export default function QuotedPost({ className, post, expanded = false }) {
  if(!post) return null
  return (
      ...
      <div className="p-2">
        <Row className="d-flex align-items-center">
          ...
          >
            ...
            >
              <Figure.Image src={post.user_profile_image_url_https} className="w-100 h-100" />
            </Figure>
          </UserLink>
          <UserLink
            to={`/user/${post.user_screen_name}`}
          ...
          <span className="text-muted">@{post.user_screen_name}</span>
          <pre className="m-0 text-muted">{" - "}</pre>
          <span className="text-muted">{formatCreatedAt(post.created_at)}</span>
        </Row>
        <Row>
          <blockquote className="mb-1">
            <PostText post={post} to={`/post/${post.id_str}`} expanded={expanded} />
          </blockquote>
        </Row>
      </div>
      <Row>
